...
const [msg,setMsg]=useState<any>(null); const [pending,setPending]=useState(false); const [lastRefreshed,setLastRefreshed]=useState(new Date().toLocaleTimeString());
const seasons=data.seasons||[]; const teams=data.teams||[]; const players=data.players||[]; const games=data.games||[]; const trades=data.trades||[]; const gameStats=data.gameStats||[]; const profiles=data.profiles||[];
...
const submit=async(action:string,payload:any,{closeOnSuccess=true}:any={})=>{setPending(true); const result=await runAction(action,payload); setPending(false); setMsg(result); if(result.ok){await refresh(); setLastRefreshed(new Date().toLocaleTimeString()); if(closeOnSuccess) setModal(null);} return result; };
...
const scoreRows=modal?.rows||(existing.length?existing.map((s:any)=>({...s,position:players.find((p:any)=>p.id===s.player_id)?.position,team_id:players.find((p:any)=>p.id===s.player_id)?.team_id})):roster.map((p:any)=>({player_id:p.id,team_id:p.team_id,position:p.position,games_played:0,goals:0,assists:0,goals_against:0})));

return <main><h1>League Operations Dashboard</h1><p className='muted'>Role: {role}{testMode?' (test mode active)':''} · Last refreshed {lastRefreshed}</p>{msg?.error&&<p>{msg.error}</p>}{msg?.ok&&<p>Saved.</p>}
...
{active==='teams'&&<TeamManager teams={seasonTeams} seasons={seasons} profiles={profiles} players={players} onAdd={()=>setModal({type:'team'})} onEdit={(t:any)=>setModal({type:'team',item:t})} onDelete={(t:any)=>submit('team_delete',{id:t.id})} />}
{active==='seasons'&&<SeasonManager seasons={seasons} currentSeasonId={data.currentSeasonId} onAdd={()=>setModal({type:'season'})} onEdit={(s:any)=>setModal({type:'season',item:s})} onDelete={(s:any)=>submit('season_delete',{id:s.id})} />}
{active==='games'&&<GameManager games={seasonGames} teams={teams} onAdd={()=>setModal({type:'game'})} onImport={()=>setModal({type:'csv',kind:'games'})} onEdit={(g:any)=>setModal({type:'game',item:g})} onBulk={(mode:string,ids:string[])=>submit('games_bulk_update',{mode,ids})} onEnterScore={(g:any)=>setModal({type:'score',item:g})} />}
...
{modal?.type==='csv'&&<CsvImportModal kind={modal.kind} seasons={seasons} targetSeason={targetSeason} setTargetSeason={setTargetSeason} pending={pending} lastResult={msg} onSubmit={(kind:any,rows:any,target:any,dry:any,existingMode:any)=>submit(kind==='players'?'import_players_csv':'import_games_csv',{rows,target_season_id:target,dry_run:dry,existing_schedule_mode:existingMode},{closeOnSuccess:!dry})} />}
{modal?.type==='player'&&(()=>{const i=modal.item||{}; const f=modal.form||{id:i.id||'',name:i.name||'',position:i.position||'',team_id:i.team_id||'',season_id:i.season_id||targetSeason}; return <div className='form-grid'><div><label>Name</label><input value={f.name} onChange={e=>setModal((m:any)=>({...m,form:{...f,name:e.target.value}}))}/></div><div><label>Team</label><select value={f.team_id} onChange={e=>setModal((m:any)=>({...m,form:{...f,team_id:e.target.value}}))}><option value=''>Unassigned</option>{seasonTeams.map((t:any)=><option key={t.id} value={t.id}>{t.name}</option>)}</select></div><button disabled={pending} onClick={()=>submit(f.id?'player_update':'player_create',f)}>Save</button></div>})()}
{modal?.type==='team'&&(()=>{const i=modal.item||{}; const f=modal.form||{id:i.id||'',name:i.name||'',season_id:i.season_id||targetSeason,captain_user_id:i.captain_user_id||'',logo_url:i.logo_url||''}; return <div className='form-grid'><input placeholder='Team name' value={f.name} onChange={e=>setModal((m:any)=>({...m,form:{...f,name:e.target.value}}))}/><select value={f.season_id} onChange={e=>setModal((m:any)=>({...m,form:{...f,season_id:e.target.value}}))}>{seasons.map((s:any)=><option key={s.id} value={s.id}>{s.name}</option>)}</select><select value={f.captain_user_id} onChange={e=>setModal((m:any)=>({...m,form:{...f,captain_user_id:e.target.value}}))}><option value=''>No captain</option>{profiles.map((p:any)=><option key={p.user_id} value={p.user_id}>{p.display_name||p.user_id}</option>)}</select><button disabled={pending} onClick={()=>submit(f.id?'team_update':'team_create',f)}>Save</button></div>})()}
{modal?.type==='season'&&(()=>{const i=modal.item||{}; const f=modal.form||{id:i.id||'',name:i.name||'',start_date:i.start_date||'',end_date:i.end_date||''}; return <div className='form-grid'><input placeholder='Season name' value={f.name} onChange={e=>setModal((m:any)=>({...m,form:{...f,name:e.target.value}}))}/><input type='date' value={f.start_date?.slice(0,10)||''} onChange={e=>setModal((m:any)=>({...m,form:{...f,start_date:e.target.value}}))}/><input type='date' value={f.end_date?.slice(0,10)||''} onChange={e=>setModal((m:any)=>({...m,form:{...f,end_date:e.target.value}}))}/><button disabled={pending} onClick={()=>submit(f.id?'season_update':'season_create',f)}>Save</button></div>})()}
{modal?.type==='game'&&(()=>{const i=modal.item||{}; const f=modal.form||{id:i.id||'',season_id:i.season_id||targetSeason,home_team:i.home_team||'',away_team:i.away_team||'',scheduled_at:toLeagueDateTimeInput(i.scheduled_at)||'',status:i.status||'SCHEDULED'}; return <div className='form-grid'><div><label>Home Team</label><select value={f.home_team} onChange={e=>setModal((m:any)=>({...m,form:{...f,home_team:e.target.value}}))}>{seasonTeams.map((t:any)=><option key={t.id} value={t.id}>{t.name}</option>)}</select></div><div><label>Away Team</label><select value={f.away_team} onChange={e=>setModal((m:any)=>({...m,form:{...f,away_team:e.target.value}}))}>{seasonTeams.map((t:any)=><option key={t.id} value={t.id}>{t.name}</option>)}</select></div><div><label>Scheduled At</label><input type='datetime-local' value={f.scheduled_at} onChange={e=>setModal((m:any)=>({...m,form:{...f,scheduled_at:e.target.value}}))}/></div><button disabled={pending} onClick={()=>submit(f.id?'game_update':'game_create',{...f,scheduled_at:new Date(f.scheduled_at).toISOString()})}>Save</button></div>})()}
{modal?.type==='score'&&<ScoreEntryModal game={scoreGame} teams={teams} players={players} homeScore={modal.homeScore??String(scoreGame?.home_score||0)} awayScore={modal.awayScore??String(scoreGame?.away_score||0)} setHomeScore={(v:any)=>setModal((m:any)=>({...m,homeScore:v}))} setAwayScore={(v:any)=>setModal((m:any)=>({...m,awayScore:v}))} rows={scoreRows} setRows={(rows:any)=>setModal((m:any)=>({...m,rows}))} onSave={()=>submit('game_score_submit',{gameId:scoreGame.id,homeScore:Number((modal.homeScore ?? scoreGame.home_score) || 0),awayScore:Number((modal.awayScore ?? scoreGame.away_score) || 0),stats:scoreRows})} />}
</AdminModal>
</main>
}
