...
def test_supabase_and_stripe_clients_are_lazy_initialized():
    supabase_admin = Path('lib/supabase/admin.ts').read_text()
    supabase_server = Path('lib/supabase/server.ts').read_text()
    supabase_client = Path('lib/supabase/client.ts').read_text()
    checkout = Path('app/api/stripe/create-checkout-session/route.ts').read_text()
    webhook = Path('app/api/stripe/webhook/route.ts').read_text()

    assert 'createAdminSupabaseClient' in supabase_admin
    assert 'createServerSupabaseClient' in supabase_server
    assert 'createBrowserSupabaseClient' in supabase_client
...
def test_register_flow_page_and_header_links_exist():
    ...
    assert 'supabase.auth.updateUser' in register_page
    assert 'createBrowserSupabaseClient' in register_page
    assert ".from('profiles')" not in register_page
